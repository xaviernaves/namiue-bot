    ...
    if (args[0]) {
        if (args[0].toString().includes(message.mentions.users.first().id)) {
            ...
            if (mention.id == message.author.id) {
                message.channel.send('Yeah look at you being beautiful today ^w^');
            } else {
                let msg, img;
                tenorSearch('anime-stare').then(obj => {
                    let random_gif = obj.results[Math.floor(Math.random() * obj.results.length)];
                    msg = `${message.author.username} intensily stares ${mention.username}`;
        ...
        message.channel.send('Yeah look at the flies around <:nm_pepo_mock:727909363490095146>')
    }
}
