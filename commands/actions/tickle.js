    ...
    if (args[0]) {
        if (args[0].toString().includes(message.mentions.users.first().id)) {
            ...
            if (mention.id == message.author.id) {
                message.channel.send('Weirdo <:nm_pepo_mock:727909363490095146>');
            } else {
                let msg, img;
                tenorSearch('anime-tickle').then(obj => {
                    let random_gif = obj.results[Math.floor(Math.random() * obj.results.length)];
                    msg = `${message.author.username} tickles ${mention.username} owo`;
        ...
        message.channel.send('Yikes...Are you tryna tickle the air bruh? <:nm_pepo_mock:727909363490095146>')
    }
}
