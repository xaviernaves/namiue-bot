    ...
    if (args[0]) {
        if (args[0].toString().includes(message.mentions.users.first().id)) {
            ...
            if (mention.id == message.author.id) {
                ...
                let msg, img;
                tenorSearch('anime-poke').then(obj => {
                    let random_gif = obj.results[Math.floor(Math.random() * obj.results.length)];
                    msg = `${message.author.username} pokes ${mention.username} owo`;
        ...
        message.channel.send('Yikes...Are you tryna poke the air bruh? <:nm_pepo_mock:727909363490095146>')
    }
}
