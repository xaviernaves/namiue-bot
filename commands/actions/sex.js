    ...
    if (args[0]) {
        if (args[0].toString().includes(message.mentions.users.first().id)) {
            ...
            if (mention.id == message.author.id) {
                message.channel.send('Go to your room pwz and make sure to get some paper broski');
            } else {
                let msg, img;
                tenorSearch('anime-sex').then(obj => {
        ...
        message.channel.send('You do you, but you need someone to secks with <:nm_pepo_mock:727909363490095146>')
    }
}
