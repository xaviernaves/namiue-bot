    ...
    if (args[0]) {
        if (args[0].toString().includes(message.mentions.users.first().id)) {
            ...
            if (mention.id == message.author.id) {
                ...
                let msg, img;
                tenorSearch('anime-cuddle').then(obj => {
                    let random_gif = obj.results[Math.floor(Math.random() * obj.results.length)];
                    msg = `${message.author.username} cuddles ${mention.username}`;
        ...
        message.channel.send('Sad times bruh <:nm_pepo_mock:727909363490095146>')
    }
}
