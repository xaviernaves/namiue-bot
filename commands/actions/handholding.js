    ...
    if (args[0]) {
        if (args[0].toString().includes(message.mentions.users.first().id)) {
            ...
            if (mention.id == message.author.id) {
                message.channel.send('You will get someone to hold hands with someday, keep at it mate');
            } else {
                let msg, img;
                tenorSearch('anime-hold-hands').then(obj => {
                    let random_gif = obj.results[Math.floor(Math.random() * obj.results.length)];
                    msg = `${message.author.username}, ${mention.username} keep it wholesome ^w^`;
